...
class KalmanTracker2D:
    """常速度模型 2D 卡尔曼滤波器：状态 [x, y, vx, vy]。"""
    def __init__(self, process_noise=25.0, measurement_noise=35.0):
        self.x = np.zeros((4, 1), dtype=np.float32)
        self.P = np.eye(4, dtype=np.float32) * 500.0
        self.Q_base = np.eye(4, dtype=np.float32) * process_noise
    ...
    def predict(self, dt):
        F = self._transition(max(dt, 1e-3))
        self.x = F @ self.x
...
# --- 新增：预测/控制融合参数 ---
KALMAN_LEAD_TIME = 0.10    # 预测提前量：提前 100ms 追踪快速横移目标
ADAPT_TIMEOUT = 0.30       # 300ms 内无法吸附，逐步增大移动步幅
ADAPT_MAX_MULT = 2.2       # 自适应最大增益
...
try:
    ...
    adapt_miss_time = 0.0
    kalman = KalmanTracker2D(process_noise=22.0, measurement_noise=30.0)
    last_target_ts = time.perf_counter()
    ...
    while True:
        ...
        if len(valid_preds) > 0:
            ...
            if best_target:
                ...
                dt = now - last_target_ts
                last_target_ts = now

                # 1) 卡尔曼更新当前观测
                kalman.predict(dt)
                kf_state = kalman.update(ex, ey)

                # 2) 做短时前馈预测：提高对横向高速目标的跟随能力
                pred_x = float(kf_state[0, 0] + kf_state[2, 0] * KALMAN_LEAD_TIME)
                pred_y = float(kf_state[1, 0] + kf_state[3, 0] * KALMAN_LEAD_TIME)

                pixel_error_x = pred_x - CENTER_X
                pixel_error_y = pred_y - dynamic_center_y - AIM_OFFSET_Y
                ...
                # 额外自适应增益：若持续超时仍未吸附，逐步放大步幅
                timeout_progress = min(1.0, adapt_miss_time / ADAPT_TIMEOUT)
                adapt_multiplier = 1.0 + timeout_progress * (ADAPT_MAX_MULT - 1.0)
                dynamic_multiplier *= adapt_multiplier
